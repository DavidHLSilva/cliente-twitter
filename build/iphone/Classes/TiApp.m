...
- (NSString *)systemUserAgent
{
  ...
  NSString *kcliente_twitter_uamiUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s", "Appc", "eler", "ator", "Tita", "nium"];
  return [NSString stringWithFormat:@"%@/%s (%@)", kcliente_twitter_uamiUserAgentPrefix, TI_VERSION_STR, currentDeviceInfo];
}
...
+ (NSDictionary *)tiAppProperties
{
  ...
  if (props == nil) {
    ...
    if (jsonData) {
      ...
      // If we have no data...
      // This should never happen on a cliente_twitter_uami app using the node.js CLI
