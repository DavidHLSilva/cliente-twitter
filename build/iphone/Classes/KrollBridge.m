...
NSString *cliente_twitter_uami$ModuleRequireFormat = @"(function(exports){"
...
@implementation cliente_twitter_uamiObject
...
- (void)dealloc
{
  ...
  RELEASE_TO_NIL(_cliente_twitter_uami);
  OSSpinLockLock(&krollBridgeRegistryLock);
...
{
  // create cliente_twitter_uami global object
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];

  // Load the "cliente_twitter_uami" object into the global scope
  NSString *basePath = (url == nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
  _cliente_twitter_uami = [[cliente_twitter_uamiObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

  TiContextRef jsContext = [kroll context];
  TiValueRef tiRef = [KrollObject toValue:kroll value:_cliente_twitter_uami];

  NSString *_cliente_twitter_uamiNS = [NSString stringWithFormat:@"T%sanium", "it"];
  TiStringRef prop = TiStringCreateWithCFString((CFStringRef)_cliente_twitter_uamiNS);
  ...
  if (preload != nil) {
    for (NSString *name in preload) {
      KrollObject *ti = (KrollObject *)[_cliente_twitter_uami valueForKey:name];
...
- (KrollWrapper *)loadCommonJSModule:(NSString *)code withSourceURL:(NSURL *)sourceURL
{
  ...
  NSString *js = [[NSString alloc] initWithFormat:cliente_twitter_uami$ModuleRequireFormat, dirname, filename, code];
...
- (KrollWrapper *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
  ...
  if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
    @throw [NSException exceptionWithName:@"org.cliente_twitter_uami.kroll"
